...
- The display may be asleep or remote; screen captures then come back black. To see the UI, run with `ATC_CAPTURE_DIR=<dir>` (renders every view off-screen to PNG) or `ATC_CAPTURE_SCRIPT=<steps.json>` (scripted steps: `{ js, wait, shot }`, run in the page; `window.__atcDev('usage' | 'launcher' | 'select:<agentId>')` switches views).
